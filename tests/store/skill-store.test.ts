...
describe("SkillStore", { concurrency: 1 }, () => {
	before(async () => {
		...
		LEGACY_PI_GLOBAL_SKILLS_DIR = path.join(
			ROOT_DIR,
			"legacy-pi-global-skills",
		);
	...
	describe("create()", () => {
		it("writes global skills to <slug>/SKILL.md", async () => {
			...
			assert.ok(
				raw.includes(
					'description: "Step-by-step approach to debugging TS errors"',
				),
			);
		...
		it("writes explicit project skills to .pi/skills/project/<project>/<slug>/SKILL.md", async () => {
			const store = await makeStore();
			const result = await store.create(
				"Release App",
		...
		it("classifies repo-specific procedures as project by default", async () => {
			...
			assert.strictEqual(
				result.skillId,
				"project:demo-project:release-demo-project",
			);
		});
		...
		it("blocks creating a similar global skill and suggests patching", async () => {
			...
			assert.strictEqual(result.suggestedAction, "patch");
			assert.ok(
				result.similarSkillIds?.includes("global:debug-typescript-errors"),
			);
		...
		it("blocks near-name global collisions even when descriptions diverge", async () => {
			...
			assert.ok(
				result.error?.includes("near-name global skill already exists"),
			);
	...
	describe("loadIndex()", () => {
		it("returns both global and project skills", async () => {
			...
			assert.ok(
				index.some((skill) => skill.skillId === "project:demo-project:skill-b"),
			);
		});
	...
	describe("move()", () => {
		it("moves a global skill into the active project scope", async () => {
			...
			await assert.rejects(
				fs.access(path.join(GLOBAL_SKILLS_DIR, "move-me", "SKILL.md")),
			);
		...
		it("moves a project skill into global scope", async () => {
			...
			await assert.rejects(
				fs.access(path.join(PROJECT_SKILLS_DIR, "repo-runbook", "SKILL.md")),
			);
		...
		it("blocks move when destination scope already has the same slug", async () => {
			const store = await makeStore();
			const globalSkill = await store.create(
				"same-name",
				"global skill",
				"body",
				"global",
			);
			const projectSkill = await store.create(
				"same-name",
				"project skill",
				"body",
				"project",
	...
	describe("migration", () => {
		...
		it("migrates flat markdown files under global skills root into SKILL.md folders", async () => {
			...
			assert.strictEqual(result.migrated, 1);
			await assert.rejects(
				fs.access(path.join(GLOBAL_SKILLS_DIR, "flat-legacy.md")),
			);
			const migrated = await readFile(
				path.join(GLOBAL_SKILLS_DIR, "flat-legacy", "SKILL.md"),
			);
		...
		it("does not write the sentinel when warnings occur, so migration can retry", async () => {
			await fs.mkdir(path.join(LEGACY_SKILLS_DIR, "broken.md"), {
				recursive: true,
			});
			const legacyFile = path.join(LEGACY_SKILLS_DIR, "legacy-skill.md");
			await fs.writeFile(
			...
			await fs.rm(path.join(LEGACY_SKILLS_DIR, "broken.md"), {
				recursive: true,
				force: true,
			});
			const second = await store.migrateLegacySkills();
	...
	describe("dynamic project context", () => {
		it("can retarget project skill creation to a new project directory", async () => {
			...
			assert.strictEqual(
				result.skillId,
				"project:another-project:deploy-another-project",
			);
			await fs.access(
				path.join(altProjectDir, "deploy-another-project", "SKILL.md"),
			);
		});
	});
});
