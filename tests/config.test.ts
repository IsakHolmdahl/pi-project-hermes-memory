...
const TEST_CONFIG_PATH = path.join(
	os.tmpdir(),
	`hermes-memory-config-test-${process.pid}.json`,
);
...
describe("loadConfig", () => {
	...
	it("overrides defaults when config file exists", () => {
		...
		assert.strictEqual(
			...
			"<memory-policy>Custom</memory-policy>",
		);
	...
	it("resolves relative memoryDir values against cwd", () => {
		fs.mkdirSync(path.dirname(TEST_CONFIG_PATH), { recursive: true });
		fs.writeFileSync(
			TEST_CONFIG_PATH,
			JSON.stringify({
		...
		const config = loadConfig(TEST_CONFIG_PATH);
		assert.strictEqual(config.memoryDir, path.resolve("custom-memory-root"));
	});
	...
	it("accepts valid memoryPolicyStyle values", () => {
		...
		for (const style of ["full", "compact", "custom", "none"] as const) {
			fs.writeFileSync(
				TEST_CONFIG_PATH,
				JSON.stringify({ memoryPolicyStyle: style }),
			);
			const config = loadConfig(TEST_CONFIG_PATH);
	...
	it("accepts valid memoryOverflowStrategy values", () => {
		...
		for (const policy of [
			...
			fs.writeFileSync(
				TEST_CONFIG_PATH,
				JSON.stringify({ memoryOverflowStrategy: policy }),
			);
			const config = loadConfig(TEST_CONFIG_PATH);
	...
	it("accepts valid sessionSearch variants", () => {
		...
		for (const variant of ["legacy", "anchors"] as const) {
			fs.writeFileSync(
				TEST_CONFIG_PATH,
				JSON.stringify({ sessionSearch: { variant } }),
			);
			const config = loadConfig(TEST_CONFIG_PATH);
	...
	it("accepts valid llmThinkingOverride values and ignores invalid ones", () => {
		...
		for (const level of [
			...
			fs.writeFileSync(
				TEST_CONFIG_PATH,
				JSON.stringify({ llmThinkingOverride: level }),
			);
			const config = loadConfig(TEST_CONFIG_PATH);
		...
		fs.writeFileSync(
			TEST_CONFIG_PATH,
			JSON.stringify({ llmThinkingOverride: "ultra" }),
		);
	...
	it("ignores blank llmModelOverride values", () => {
		...
		fs.writeFileSync(
			TEST_CONFIG_PATH,
			JSON.stringify({ llmModelOverride: "   " }),
		);
		const config = loadConfig(TEST_CONFIG_PATH);
		assert.strictEqual(config.llmModelOverride, undefined);
	});
	...
	it("maps legacy autoConsolidate boolean to memoryOverflowStrategy when strategy is absent", () => {
		...
		fs.writeFileSync(
			TEST_CONFIG_PATH,
			JSON.stringify({ autoConsolidate: false }),
		);
		let config = loadConfig(TEST_CONFIG_PATH);
		...
		fs.writeFileSync(
			TEST_CONFIG_PATH,
			JSON.stringify({ autoConsolidate: true }),
		);
		config = loadConfig(TEST_CONFIG_PATH);
		assert.strictEqual(config.autoConsolidate, true);
		assert.strictEqual(config.memoryOverflowStrategy, "auto-consolidate");
	...
	it("accepts correction pattern string arrays including empty arrays", () => {
		...
		assert.deepStrictEqual(config.correctionNegativePatterns, [
			"^custom negative",
		]);
