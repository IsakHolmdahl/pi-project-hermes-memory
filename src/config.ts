...
const SESSION_SEARCH_VARIANTS: readonly SessionSearchVariant[] = [
	"legacy",
	"anchors",
];
...
function isMemoryOverflowStrategy(
	value: unknown,
): value is MemoryOverflowStrategy {
	return (
		typeof value === "string" &&
...
function isSessionSearchVariant(value: unknown): value is SessionSearchVariant {
	return (
		typeof value === "string" &&
		SESSION_SEARCH_VARIANTS.includes(value as SessionSearchVariant)
	);
}
...
export function loadConfig(configPath?: string): MemoryConfig {
	const pathToLoad = configPath ?? resolveLocalConfigPath();
	try {
		if (fs.existsSync(pathToLoad)) {
			...
			if (
				parsed.memoryMode === "policy-only" ||
				parsed.memoryMode === "legacy-inject"
			)
			...
			if (typeof parsed.userCharLimit === "number")
				config.userCharLimit = parsed.userCharLimit;
			if (typeof parsed.nudgeInterval === "number")
				config.nudgeInterval = parsed.nudgeInterval;
			...
			if (typeof parsed.reviewEnabled === "boolean")
				config.reviewEnabled = parsed.reviewEnabled;
			if (typeof parsed.flushOnCompact === "boolean")
				config.flushOnCompact = parsed.flushOnCompact;
			if (typeof parsed.flushOnShutdown === "boolean")
				config.flushOnShutdown = parsed.flushOnShutdown;
			if (typeof parsed.flushMinTurns === "number")
				config.flushMinTurns = parsed.flushMinTurns;
			...
			if (typeof parsed.nudgeToolCalls === "number")
				config.nudgeToolCalls = parsed.nudgeToolCalls;
			if (typeof parsed.projectCharLimit === "number")
				config.projectCharLimit = parsed.projectCharLimit;
			if (typeof parsed.memoryDir === "string") {
				const normalizedMemoryDir = normalizeConfiguredMemoryDir(
					parsed.memoryDir,
				);
			...
			if (hasMemoryOverflowStrategy) {
				config.autoConsolidate =
					config.memoryOverflowStrategy === "auto-consolidate";
			} else if (hasLegacyAutoConsolidate) {
				config.memoryOverflowStrategy = config.autoConsolidate
					? "auto-consolidate"
					: "reject";
			}
			return config;
