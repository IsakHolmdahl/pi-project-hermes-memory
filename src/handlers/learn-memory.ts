	...
	pi.registerCommand("learn-memory-tool", {
		...
		handler: async (_args, ctx: ExtensionCommandContext) => {
			...
			if (section.startsWith("🔄")) {
				...
				lines.push("");
				lines.push(
					"  1. Session starts     → Compact memory policy is injected",
				);
				lines.push(
					"  2. During conversation → Agent searches memory when useful",
				);
				lines.push(
					"  3. Agent saves        → Markdown memory + best-effort SQLite sync",
				);
				lines.push("  4. Every 10 turns     → Background review saves items");
				lines.push(
					"  5. On correction      → Immediate save as [correction] category",
				);
			...
			if (section.startsWith("🏗️")) {
				...
				lines.push("  ┌─────────────────────────────────────┐");
				lines.push("  │ .pi/hermes-memory/MEMORY.md         │");
				lines.push("  │ .pi/hermes-memory/USER.md           │");
				lines.push("  │ .pi/hermes-memory/failures.md       │");
				...
				lines.push("");
				lines.push(
					"  Legacy mode can still inject full memory blocks for users",
				);
			...
			if (section.startsWith("❓")) {
				...
				lines.push("    → Files at <cwd>/.pi/hermes-memory/");
			}
