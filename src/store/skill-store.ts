 ...
 *
 * Global skills live in <cwd>/.pi/skills/<slug>/SKILL.md.
 * Project skills live in <cwd>/.pi/skills/project/<project>/<slug>/SKILL.md.
 */
...
import { resolveLocalSkillsDir } from "../paths.js";
...
export class SkillStore {
	...
	constructor(options: SkillStoreOptions = {}) {
		const skillsDir = options.globalSkillsDir ?? resolveLocalSkillsDir();
		this.globalSkillsDir = skillsDir;
		this.projectSkillsDir = options.projectSkillsDir ?? null;
		this.projectName = options.projectName ?? null;
		this.legacySkillsDir =
			options.legacySkillsDir ?? path.join(skillsDir, "legacy", "skills");
		this.legacyPiGlobalSkillsDir =
			options.legacyPiGlobalSkillsDir ??
			path.join(skillsDir, "legacy", "skills");
		this.migrationSentinelPath =
			options.migrationSentinelPath ??
			path.join(skillsDir, ".skills-migrated-to-extension-storage");
	}
	...
	private async migrateLegacyPiGlobalSkillDirs(
		result: LegacySkillMigrationResult,
	): Promise<void> {
		if (
			path.resolve(this.legacyPiGlobalSkillsDir) ===
			path.resolve(this.globalSkillsDir)
		)
			return;
		if (!(await exists(this.legacyPiGlobalSkillsDir))) return;

		const entries = await fs.readdir(this.legacyPiGlobalSkillsDir, {
			withFileTypes: true,
		});
	...
	async patch(
		...
		if (!newContent)
			return { success: false, error: "New content is required for patch." };
	...
	private async scanScope(
		...
		const walk = async (dir: string, isRoot: boolean): Promise<void> => {
			...
			for (const entry of dirs) {
				...
				if (scope === "global" && isRoot && entry.name === "project") continue;
